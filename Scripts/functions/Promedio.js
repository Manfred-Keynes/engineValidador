/**
 * Promedio.js - Funcion Promedio
 *
 * Arrow IIFE Pattern - Modulo encapsulado
 * Namespace: EF.Functions.Promedio
 *
 * Calcula el promedio de todos los valores proporcionados. Permite agregar parametros dinamicamente.
 * Ejemplo: #Promedio([Campo1],[Campo2],[Campo3])#
 */

const FunctionPromedioModule = (() => {
    'use strict';
    ...
    // ===== CLASE DE FUNCION =====
    class FunctionPromedio extends EF.Core.FunctionBase {
        // Metadata estatica
        static functionName = 'Promedio';
        static functionId = 'Promedio';
        static icon = 'fa-chart-line';
        static description = 'Calcula el promedio de los valores proporcionados';
        ...
        generateForm() {
            ...
            <div style="text-align: center; margin: 12px 0;">
                <button type="button" onclick="FunctionPromedio.addParam()"
        ...
        static addParam() {
            ...
            <div class="mini-builder-container" style="margin-bottom: 20px; position: relative;">
                ...
                <button type="button" onclick="FunctionPromedio.removeParam(this)"
        ...
        buildPreview(params) {
            const values = (params || []).filter(p => p && p.toString().trim() !== '');
            if (values.length === 0) return '#Promedio(...)#';

            const preview = values.join(',');
            if (preview.length > 28) {
                return `#Promedio(${preview.substring(0, 25)}...)#`;
            }
            return `#Promedio(${preview})#`;
        }
        ...
        validate(params) {
            ...
            if (values.length < 1) {
                return {
                    valid: false,
                    errors: ['Promedio requiere al menos 1 parametro']
                };
            }
        ...
        parseExpression(expression) {
            const match = expression.match(/#Promedio\((.*)\)#/);
    ...
    EF.Core.Registry.register(FunctionPromedio);

    // Registrar en namespace
    EF.Functions.Promedio = FunctionPromedio;

    // Compatibilidad global
    window.FunctionPromedio = FunctionPromedio;

    console.log('[EF] FunctionPromedio cargado');

    return FunctionPromedio;
})();
