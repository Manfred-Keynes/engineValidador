/**
 * Suma.js - Funcion Suma
 *
 * Arrow IIFE Pattern - Modulo encapsulado
 * Namespace: EF.Functions.Suma
 *
 * Suma todos los valores proporcionados. Permite agregar parametros dinamicamente.
 * Ejemplo: #Suma([Campo1],[Campo2],[Campo3])#
 */

const FunctionSumaModule = (() => {
    'use strict';
    ...
    // ===== CLASE DE FUNCION =====
    class FunctionSuma extends EF.Core.FunctionBase {
        // Metadata estatica
        static functionName = 'Suma';
        static functionId = 'Suma';
        static icon = 'fa-plus';
        static description = 'Suma todos los valores proporcionados';
        ...
        generateForm() {
            ...
            <div style="text-align: center; margin: 12px 0;">
                <button type="button" onclick="FunctionSuma.addParam()"
        ...
        static addParam() {
            ...
            <div class="mini-builder-container" style="margin-bottom: 20px; position: relative;">
                ...
                <button type="button" onclick="FunctionSuma.removeParam(this)"
        ...
        buildPreview(params) {
            const values = (params || []).filter(p => p && p.toString().trim() !== '');
            if (values.length === 0) return '#Suma(...)#';

            const preview = values.join(',');
            if (preview.length > 30) {
                return `#Suma(${preview.substring(0, 27)}...)#`;
            }
            return `#Suma(${preview})#`;
        }
        ...
        validate(params) {
            ...
            if (values.length < 1) {
                return {
                    valid: false,
                    errors: ['Suma requiere al menos 1 parametro']
                };
            }
        ...
        parseExpression(expression) {
            const match = expression.match(/#Suma\((.*)\)#/);
    ...
    EF.Core.Registry.register(FunctionSuma);

    // Registrar en namespace
    EF.Functions.Suma = FunctionSuma;

    // Compatibilidad global
    window.FunctionSuma = FunctionSuma;

    console.log('[EF] FunctionSuma cargado');
